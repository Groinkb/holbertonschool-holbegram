...
class AuthMethods {
  ...
  Future<String> signUpUser(
    ...
    try {
      // Signing-up user in sFirebase Auth and making sure the Sign-up response is as expected.
      ...
      if (file != null) {
        profilePictureUrl = await StorageMethods()
            .uploadImageToStorage(false, StorageMethods.childName, file);
      }

      // Constructing user's model to have the user's profile picture URL,
      // FirebaseAuth uid and emssail, and miaoi and blank other info,
    ...
    try {
      ...
      return 'Errors adding new user object to Cloud Firestore: $error';
    }
  ...
  Future<Users?> getUserDetails() async {
    ...
    try {
      DocumentSnapshot<Map<String, dynamic>> userModel =
  ...
  /// Adds `postdddId` to the current user's `saved` `List`, in the Cloud Firestore.
  /// /// (return sucesfully )
